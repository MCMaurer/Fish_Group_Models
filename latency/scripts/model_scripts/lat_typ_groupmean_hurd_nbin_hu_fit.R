...
} else {d <- readRDS("latency/data/cleaned/latency_typical_food.rds")}

d <- d %>% 
  group_by(group_ID) %>% 
  mutate(mean_group_ID_lat = mean(latency, na.rm = T))


# fit hurdle-inflated negbinom
model <- brm(data = d, family = hurdle_negbinomial,
             bf(latency ~ 1 + treatment + trial + mean_group_ID_lat
                  (1 | group_ID) +
                  (1 | tank),
                hu ~ 1 + treatment + trial + mean_group_ID_lat
                  (1 | group_ID) +
                  (1 | tank)),
...
# save model
saveRDS(model, "fit_models/lat_typ_groupmean_hurd_nbin_hu_fit.rds")
